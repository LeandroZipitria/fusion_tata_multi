...
# Calculates de distance (in kms) between all supermarkets in the data set. The result is an NxN matrix with the 
# distance between all supers.

## Load Database
Establecimientos <- read.csv("2017.Establecimientos 30-08.csv") %>% as_tibble()

## Calculate euclidian distance
df <- Establecimientos[,c(1,9,10)] %>% as_tibble() # Just supermarket and X, Y UTM

# Need to add those numbers that are not in the supermarket database in order for rows to coincide
# with supermarkets
...
# Calculates which stores have a supermarket of chain Tata at less than 0.5, 1 and 2 km
